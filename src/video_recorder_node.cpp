...
public:
    ...
    : Node("video_recorder_node"), controller_(controller) {
        set_record_sub_ = this->create_subscription<std_msgs::msg::String>(
            "set_record", 10,
            std::bind(&VideoRecorderNode::set_record_callback, this, std::placeholders::_1)
        );
        unset_record_sub_ = this->create_subscription<std_msgs::msg::String>(
            "unset_record", 10,
            std::bind(&VideoRecorderNode::unset_record_callback, this, std::placeholders::_1)
        );

        start_record_sub_ = this->create_subscription<std_msgs::msg::Empty>(
            "start_recording", 10,
            std::bind(&VideoRecorderNode::start_record_callback, this, std::placeholders::_1)
        );
        stop_record_sub_ = this->create_subscription<std_msgs::msg::Empty>(
            "stop_recording", 10,
            std::bind(&VideoRecorderNode::stop_record_callback, this, std::placeholders::_1)
        );
        set_data_dir_sub_ = this->create_subscription<std_msgs::msg::String>(
            "set_data_directory", 10,
            std::bind(&VideoRecorderNode::set_data_dir_callback, this, std::placeholders::_1)
        );
    }
private:
...
int main(int argc, char* argv[])
{
    ...
    if (file.is_open()) {
        ...
        if (reader.parse(file, root)) {
            ...
            std::cerr << "Failed to parse configuration file: " << reader.getFormattedErrorMessages() << std::endl;
            return 1;
        }
    } else {
